...
function main()
    ...
    y_PH = solve_progressivehedging(pbcvar, maxiter=80, maxtime=60, printstep=2, hist=hist)
    ...
    y_synch = solve_randomized_sync(pbcvar, maxiter=80, maxtime=60, printstep=10)
    println("\nSynchronous solve output is:")
    display(y_synch)
    
    # #########################################################
    # ## Problem solve: asynchronous (parallelized) version of PH
    y_asynch = solve_randomized_async(pbcvar, maxiter=80, maxtime=60, printstep=10, hist=hist)
